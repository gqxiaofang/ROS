...
bool ScanDataReceiver::handleNextPacket()
{
    ...
    if( p->header.packet_number == 1 || scan_data_.empty() )
    {
        ...
        // ROS_WARN_STREAM("add scan data = " <<  ros::Time::now());
        // ROS_WARN_STREAM("[Packet2]" << scandata.headers[num_scan_points - 1].timestamp_raw);
    }
...
ScanData ScanDataReceiver::getFullScan()
{
    ...
    // auto deltastamp = ros::Time::now()- stamp;
    // if(deltastamp > ros::Duration(0.0001))
    // {
    //     ROS_WARN_STREAM("delta" << ros::Time::now()- stamp);
    // }
    ScanData data;
